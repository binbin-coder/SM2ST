...
def get_link_list_df(VIM, gene_names=None, regulators='all', maxcount='all'):
    """
    Extract the regulatory links from the VIM matrix of GENIE3 and return them as a DataFrame 
    Parameters:
        VIM: numpy array, the weight importance matrix returned by GENIE3
        gene_names: list of gene names
        regulators: list of candidate regulatory factors, 'all' indicates all genes
        maxcount: maximum number of links returned, 'all' indicates all 
    Return:
         DataFrame, containing three columns: regulator (regulatory factor), target (target gene), weight (weight) 
    """
    # 输入检查
    if not isinstance(VIM, ndarray):
        raise ValueError('VIM must be a square array')
    ...
    if gene_names is not None and len(gene_names) != ngenes:
        raise ValueError('gene_names length must match VIM dimensions')
        
    if regulators != 'all' and gene_names is None:
        raise ValueError('gene_names must be provided when regulators is specified')
    ...
    # 提取所有调控链接（排除自调控）
    vInter = [(i, j, score) for (i, j), score in ndenumerate(VIM) 
    ...
    # 确定返回数量
    nInter = len(vInter_sort)
    if isinstance(maxcount, int) and maxcount < nInter:
        vInter_sort = vInter_sort[:maxcount]
    
    # 构建DataFrame
    data = []
    for TF_idx, target_idx, score in vInter_sort:
        TF_idx, target_idx = int(TF_idx), int(target_idx)
        reg_name = gene_names[TF_idx] if gene_names else f'G{TF_idx+1}'
        tgt_name = gene_names[target_idx] if gene_names else f'G{target_idx+1}'
        data.append({'regulator': reg_name, 'target': tgt_name, 'weight': score})
