...
# class spatial_transfor(nn.Module):
#     def __init__(self, device, grid_size):
#         # grid_size = [padded_HE_height, padded_HE_width] list
#         super(spatial_transfor, self).__init__()

#         # 初始化缩放参数，支持镜像
#         self.w_x = nn.Parameter(torch.tensor(1.0, dtype=torch.float32, requires_grad=True, device=device))
#         self.w_y = nn.Parameter(torch.tensor(1.0, dtype=torch.float32, requires_grad=True, device=device))

#         # 初始化平移参数
#         self.bias = nn.Parameter(torch.tensor([[0.0], [0.0]], dtype=torch.float32, requires_grad=True, device=device))
...
#     def forward(self, x):
#         # 构造仿射变换矩阵
#         # 镜像通过设置 w_x 或 w_y 为负值实现
#         self.diagonal_tensor = torch.diag(torch.stack([self.w_x, self.w_y]))
...
#         super(spatial_transfor,self).__init__()
#         # st data
#         self.theta = Parameter(torch.tensor([[[-1, 0, 0], [0, 1, 0]]], dtype=torch.float32, device=device))
#         # gzlab data
#         # self.theta = Parameter(torch.tensor([[[0, -1, 0], [1, 0, 0]]], dtype=torch.float32, device=device))
#         self.grid_size=[1,1]+grid_size
...
#     def forward(self, x):
#         grid = F.affine_grid(self.theta, size=torch.Size(self.grid_size), align_corners=False)
#         warp_img = F.grid_sample(x, grid, align_corners=False)
...
class fit():
    def __init__(self,num_epochs=500,lr=0.05,device=torch.device("cpu")):
        ...
        #     transforms.Resize(224),          # 调整图像大小到 224×224
